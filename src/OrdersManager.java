...
public class OrdersManager {
    ...
    void removeUnprofitableOrders() {
        ...
        for (String name : customersOrders.keySet()){ // перебираем ключи из полученного сета ключей
            double ordersSum = 0;
            for (Double order : customersOrders.get(name)) { // перебираем значения ключа
                ordersSum += order; //суммируем все значения
            }
            if (ordersSum < 5000) { // если сумма меньше 5000 вносим имя в список
                    names.add(name);
            }

        }
        for (String name : names){ // перебираем ключи хэш-мапы
